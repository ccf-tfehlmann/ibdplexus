...
The list of pre-programmed ICD10 and ICD9 codes is below. The text
search term column specifies the words and phrases for the
pre-programmed categories when both text\_search and code\_search are
...
filter through the EMR using both ICD codes and keyword text search.
Currently, the function will only run with both of these options if the
input is one of the pre-programmed condition categories. For example, if
text\_search = T and code\_search = T with inclusion = “COLORECTAL
CANCER”, the function will return all results that match the ICD 10 &
ICD 9 codes in the colorectal cancer condition category, as well as any
results that have any of the specified text strings in the description
of the diagnosis but are not one of the specified codes (ie. a SNOMED
code might return for a combined text and code search).
