...
wpcdai <- function(observations, encounter, labs) {
  ...
  visit$`PCDAI - EIM` <- sapply(1:nrow(visit), function(x) {
    if (all(c(
      visit$`PCDAI - EIM: DEFINITE ARTHRITIS`[x], visit$`PCDAI - EIM: E NODOSUM`[x],
      visit$`PCDAI-EIM:FEVER>38.5C FOR 3 DAYS OVER PAST WEEK`[x], visit$`PCDAI - EIM: ORAL ULCERS`[x],
      visit$`PCDAI - EIM: P GANGRENOSUM`[x], visit$`PCDAI - EIM: UVEITIS`[x]
    ) %in% "")) {
      NA
    } else if (all(c(
      visit$`PCDAI - EIM: DEFINITE ARTHRITIS`[x], visit$`PCDAI - EIM: E NODOSUM`[x],
  ...
  visit <- visit %>%
    rowwise() %>%
    mutate(wpcdai = sum(c(
      `PCDAI - MOST SEVERE ABDOMINAL PAIN PAST 7 DAYS`, `PCDAI-PT FUNCTING,GENERAL WELLBEING PAST 7 DAY`,
