...
sparc_summary <- function(data,
  ...
  mes <- data$procedures %>%
    ...
    slice(which.min(abs(datediff))) %>%
    distinct(
      DEIDENTIFIED_MASTER_PATIENT_ID, index_date, MAX_EXTENT_ACTIVE_DISEASE, Rectum, `Sigmoid colon`, `Right colon`,
  ...
  es <- bind_rows(ses, mes) %>%
    select(
      DEIDENTIFIED_MASTER_PATIENT_ID, index_date, Ileum, `Descending colon`, `Left colon`, `Rectum`, `Right colon`, `Sigmoid colon`, `Transverse colon`,
  ...
  scdai <- cohort %>%
    ...
    mutate(across(c(A, B, G), ~ case_when(
      . == "None" ~ 0,
      . == "Mild" ~ 1,
      . == "Moderate" ~ 2,
      . == "Severe" ~ 3,
  ...
  mayo <- cohort %>%
    ...
    mutate(across(c(T, R, S), ~ case_when(
      . == "Normal" ~ 0,
    ...
    mutate(
      MAYO_6PT = Stool.Freq.Score + Rectal.Bleeding.Score,
  ...
  cohort <- cohort %>%
    ...
    mutate(DISEASE_ACTIVITY = case_when(
      is.na(DISEASE_ACTIVITY) & PGA == 0 ~ "Remission",
