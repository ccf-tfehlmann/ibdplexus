...
#'
#' @return A patient list of with all relevant EMR lab result indicators
#' @export
#'
#' @examples
#'
...
lab_emr_patient <- function(data,
                            labtype,
                            codetype=c("LOINC","KEYWORDS","ALL"),
                            positive_elevated=FALSE,
                            datecutoff = NULL,
                            index_range = 36500){
  if(positive_elevated==TRUE&is.null(datecutoff)==TRUE){
    labdatapos=extract_lab_emr(data,labtype,codetype,positive_elevated)
  }
  if(positive_elevated==TRUE&is.null(datecutoff)==FALSE){
    labdatapos=extract_lab_emr(data,labtype,codetype,positive_elevated,datecutoff,index_range)
  }
  if(is.null(datecutoff)==TRUE){
    labdataall=extract_lab_emr(data,labtype,codetype,FALSE)
  }
  if(is.null(datecutoff)==FALSE){
    labdataall=extract_lab_emr(data,labtype,codetype,FALSE,datecutoff,index_range)
  }
  labtype1=toupper(labtype)
  if(labtype1=="CRP"){
    patientlab=data$demographics %>%
      mutate(crp_all=ifelse(DEIDENTIFIED_MASTER_PATIENT_ID %in% labdataall$DEIDENTIFIED_MASTER_PATIENT_ID,1,0))
    if(positive_elevated==TRUE){
      patientlab=patientlab %>%
        mutate(crp_pos=ifelse(DEIDENTIFIED_MASTER_PATIENT_ID %in% labdatapos$DEIDENTIFIED_MASTER_PATIENT_ID,1,0))
    }
  }
  if(labtype1=="HS CRP"){
    patientlab=data$demographics %>%
      mutate(hscrp_all=ifelse(DEIDENTIFIED_MASTER_PATIENT_ID %in% labdataall$DEIDENTIFIED_MASTER_PATIENT_ID,1,0))
    if(positive_elevated==TRUE){
      patientlab=patientlab %>%
        mutate(hscrp_pos=ifelse(DEIDENTIFIED_MASTER_PATIENT_ID %in% labdatapos$DEIDENTIFIED_MASTER_PATIENT_ID,1,0))
    }
  }
  if(labtype1=="NS CRP"){
    patientlab=data$demographics %>%
      mutate(nscrp_all=ifelse(DEIDENTIFIED_MASTER_PATIENT_ID %in% labdataall$DEIDENTIFIED_MASTER_PATIENT_ID,1,0))
    if(positive_elevated==TRUE){
      patientlab=patientlab %>%
        mutate(nscrp_pos=ifelse(DEIDENTIFIED_MASTER_PATIENT_ID %in% labdatapos$DEIDENTIFIED_MASTER_PATIENT_ID,1,0))
    }
  }
  if(labtype1=="FCAL"){
    patientlab=data$demographics %>%
      mutate(fcal_all=ifelse(DEIDENTIFIED_MASTER_PATIENT_ID %in% labdataall$DEIDENTIFIED_MASTER_PATIENT_ID,1,0))
    if(positive_elevated==TRUE){
      patientlab=patientlab %>%
        mutate(fcal_pos=ifelse(DEIDENTIFIED_MASTER_PATIENT_ID %in% labdatapos$DEIDENTIFIED_MASTER_PATIENT_ID,1,0))
    }
  }
