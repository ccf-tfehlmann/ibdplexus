...
qorus_summary <- function(datadir,
  ...
  if ("DATE_OF_CONSENT" %in% index_info) {
    cohort <- demo %>% mutate(index_date = DATE_OF_CONSENT)
  } else {
    cohort <- index_info %>% left_join(demo)
  }

  # DIAGNOSIS ----


  dx <- extract_diagnosis(data$diagnosis, "QORUS")
  ...
  mayo6 <- data$observations %>%
    ...
    mutate(result = ifelse(is.na(DESCRIPTIVE_SYMP_TEST_RESULTS), TEST_RESULT_NUMERIC, DESCRIPTIVE_SYMP_TEST_RESULTS)) %>%
    mutate(result = case_when(
      result %in% c("Normal", "Normal (same as when I am well)") ~ 0,
