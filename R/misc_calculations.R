...
calculate_location_az <- function(observations,encounter,window = 90)
{
  ...
  cdloc <- cd_locations_wide %>%
    ...
    mutate("Pure Ileal Phenotype" = if_else(`Colonic involvement` == "No" &  `Ileal Phenotype` == "Yes","Yes",
                                            if_else(`Colonic involvement` == "Unknown" |  `Ileal Phenotype` == "Unknown", "Unknown",  "No"  ) )  ) %>%
    ungroup()  %>%  mutate(VISIT_ENCOUNTER_START_DATE = dmy(VISIT_ENCOUNTER_START_DATE)) %>%
...
# Under this rule, a colonic and ileal record would be combined to ilealcolonic
  ...
  location_summarizer <- function(x) {case_when( any(x == "Ilealcolonic") ~ "Ilealcolonic",
  ...
  upper_gi_summarizer <- function(x) {case_when(any(x== "Yes") ~ "Yes",
                                                any(x == "Unknown") ~ "Unknown",
                                                all(is.na(x)) ~ "Unknown",
                                                TRUE ~ "No")}
  ...
  cdloc %>%
    ...
    summarise("CD Location" = location_summarizer(`CD Location`),
...
{
  case_when(any(x=="Yes") ~ "Yes", # take the worst outcome if there is any record of the phenotype assume it is present
            all(x=="No") ~ "No",
            ...
            TRUE ~ paste(x,collapse = ";") ) # return any unhandled cases unmodified.
}
...
{
  stricture_terms <- c("Phenotype - Anal Stricture",
  ...
  fistula_terms <-  c("Phenotype - Enterocutaneous Fistula",
  ...
  perianal_terms <- c("Rectovaginal Fistula",
                      "Perianal Fistula",
  ...
  cdpt <- observations %>%
    ...
    mutate(`DESCRIPTIVE_SYMP_TEST_RESULTS` =  case_when(OBS_TEST_CONCEPT_NAME == "Anal Fistula" & `DESCRIPTIVE_SYMP_TEST_RESULTS` == "Active drainage" ~"Yes",
                                                        ...
                                                        TRUE ~ `DESCRIPTIVE_SYMP_TEST_RESULTS`)) %>% # map terms used under concept anal fistula to yes/no
    pivot_wider(id_cols = c(DEIDENTIFIED_MASTER_PATIENT_ID, OBS_TEST_RESULT_DATE),
